...
- You will need to have an upstream remote configured to use to the [vmware-tanzu/velero](https://github.com/adi-bhardwaj/velero-modified) repository.
  You can check this using `git remote -v`.
  The release script ([`tag-release.sh`](https://github.com/adi-bhardwaj/velero-modified/blob/main/hack/release-tools/tag-release.sh)) will use `upstream` as the default remote name if it is not specified using the environment variable `REMOTE`.
...
1. Create the "Upgrade to $major.minor" page if it does not already exist ([example](https://velero.io/docs/v1.5/upgrade-to-1.5/)).
   If it already exists, update any usage of the previous version string within this file to use the new version string instead ([example](https://github.com/adi-bhardwaj/velero-modified/pull/2941/files#diff-d594f8fd0901fed79c39aab4b348193d)).
...
1. Publish the release
	- Navigate to the draft GitHub release at https://github.com/adi-bhardwaj/velero-modified/releases and edit the release.
	...
	- Verify that GitHub has built and pushed all the images (it takes a while): https://github.com/adi-bhardwaj/velero-modified/actions
...
[3]: https://github.com/adi-bhardwaj/velero-modified/tree/main/test/e2e
