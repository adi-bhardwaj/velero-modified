...
1. Clone the [Velero project](https://github.com/adi-bhardwaj/velero-modified) repository
   locally
...
Note: to use MinIO as an object storage, you will need to use the [`AWS` plugin](https://github.com/adi-bhardwaj/velero-modified-plugin-for-aws), and configure the storage location with the `spec.provider` set to `aws` and the `spec.config.region` set to `minio`. Example:
```
spec:
  config:
    region: minio
...
1) As a standalone MinIO instance running locally in a Docker container
    ...
    See [these instructions](https://github.com/adi-bhardwaj/velero-modified/discussions/3381) to run MinIO locally on your computer, as a standalone as opposed to running it on a Pod.
