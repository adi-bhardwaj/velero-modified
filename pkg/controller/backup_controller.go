...
func (c *backupController) processBackup(key string) error {
	...
	if err := c.runBackup(request, "", true); err != nil {
...
func (c *backupController) runBackup(backup *pkgbackup.Request, mountPath string, shouldUploadMetadata bool) error {
	...
	if shouldUploadMetadata {
		...
		backupLog.Info("Skipping setting up of backup store")
	}
	...
	if shouldUploadMetadata && features.IsEnabled(velerov1api.CSIFeatureFlag) {
	...
	if shouldUploadMetadata {
		...
		backupLog.Info("Skipping setting up of object store to persist the data")
	}

	if errs := persistBackup(backup, backupFile, logFile, backupStore,
		c.logger.WithField(Backup, kubeutil.NamespaceAndName(backup)), volumeSnapshots, volumeSnapshotContents,
		mountPath, shouldUploadMetadata); len(errs) > 0 {
...
func persistBackup(backup *pkgbackup.Request,
	...
	mountPath string, shouldUploadMetadata bool,
) []error {
	persistErrs := []error{}
	...
	if shouldUploadMetadata {
		...
		log.Info("persisting backup metadata locally at mount path: " + mountPath)
		const backupDirName = "backup"
		const backupJsonFileName = "backupJson.tar.gz"
		const backupContentsFileName = "backupContents.tar.gz"
		...
		log.Info("writing backupJSON to the local storage")
		if err := ioutil.WriteFile(currentBackupDir + "/" + backupJsonFileName, backupJSON.Bytes(), 0644); err != nil {
			log.WithError(err).Error("failed to write the backupJSON data to local storage")
			persistErrs = append(persistErrs, err)
		}
		log.Info("writing backupContent to the local storage")
		output, err := os.Create(currentBackupDir + "/" + backupContentsFileName)
		if err != nil {
		...
		outputLog, err := os.Create(currentBackupDir + "/" + backupLogFileName)
		if err != nil {
