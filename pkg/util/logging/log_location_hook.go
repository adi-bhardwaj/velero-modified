...
const (
	...
	veleroPackage           = "github.com/adi-bhardwaj/velero-modified/"
...
func (h *LogLocationHook) Fire(entry *logrus.Entry) error {
	...
	// skip 2 frames:
	//   runtime.Callers
	//   github.com/adi-bhardwaj/velero-modified/pkg/util/logging/(*LogLocationHook).Fire
...
func removeVeleroPackagePrefix(file string) string {
	if index := strings.Index(file, veleroPackage); index != -1 {
		// strip off .../github.com/adi-bhardwaj/velero-modified/ so we just have pkg/...
