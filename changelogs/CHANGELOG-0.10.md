...
need to be updated for v0.10.
    - The [`ObjectStore.ListCommonPrefixes`](https://github.com/adi-bhardwaj/velero-modified/blob/main/pkg/cloudprovider/object_store.go#L50) signature has changed to add a `prefix` parameter.
    - Registering plugins has changed. Create a new plugin server with the `NewServer` function, and register plugins with the appropriate functions. See the [`Server`](https://github.com/adi-bhardwaj/velero-modified/blob/main/pkg/plugin/server.go#L37) interface for details.
